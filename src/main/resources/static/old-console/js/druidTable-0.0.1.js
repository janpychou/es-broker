      ...
      for (var r in this.getRows()) {
        ...
        for (var field in fieldNames) {
          ...
          if (row.hasOwnProperty(field)) {
            var content = this.getCell(r, field);
            var tdContent = content;
//            if(content.indexOf('http://') == 0 || content.indexOf('https://') == 0) {
            if(reg.test(content)) {
                tdContent = '<a target="_blank" href="' + content + '">' + content + '</a>';
            }
            html += "<td " + "class=\"" + field.replace(' ', '_').toLowerCase() + "\" title=\"" + content + "\">" + tdContent + "</td>";
          } else {
            html += "<td></td>";
          }
        }
